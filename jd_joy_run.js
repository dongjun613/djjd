...
//下面给出好友邀请助力的示例填写规则
let invite_pins = ['dongjun613,dongjuncly,jd_5ae6176884a9d'];
//下面给出好友赛跑助力的示例填写规则
let run_pins = ['dongjun613,dongjuncly,jd_5ae6176884a9d'];
//friendsArr内置太多会导致IOS端部分软件重启,可PR过来(此处目的:帮别人助力可得30g狗粮)
let friendsArr = ["dongjun613", "dongjuncly", "jd_5ae6176884a9d"]
    ...
    if (cookiesArr[i]) {
      ...
      if ($.isNode()) {
        if (process.env.JOY_RUN_HELP_MYSELF) {
          ...
          let fixPins = run_pins.splice(run_pins.indexOf('dongjun613'), 1);
...
function getFriendPins() {
  return new Promise(resolve => {
    $.get({
      url: "http://47.106.222.216:18880/share_code/friendPins.json",
      headers:{
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
      },

...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN(), res2 = await updateTuanIdsCDN("https://gitee.com/dongjun613/djjd2/raw/master/share_code/factory.json")
  if (!res) res = await updateTuanIdsCDN('https://gitee.com/dongjun613/djjd2/raw/master/share_code/jd_updateFactoryTuanId.json');
...
function updateTuanIdsCDN(url = 'https://gitee.com/dongjun613/djjd2/raw/master/share_code/jd_updateFactoryTuanId.json') {
  return new Promise(async resolve => {
    const options = {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://47.106.222.216:18880/api/v2/jd/jxfactory/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
  ...
  return new Promise(async resolve => {
    ...
    if (!tuanActiveId) {
      ...
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          await updateTuanIdsCDN('https://gitee.com/dongjun613/djjd2/raw/master/share_code/jd_updateFactoryTuanId.json');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {

...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/lsh26/share_code/main/shop.json1');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://gitee.com/star267/share-code/raw/master/shop.json1');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
        autoCode = getRandomArrayElements(res,1)[0];
    }
    autoCode = 'S6qItF0Rcq1LWIw';

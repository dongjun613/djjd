...
!(async () => {
    ...
    let res = await getAuthorShareCode('http://47.106.222.216:18880/share_code/jxcfd.json')
    if (!res) {
        $.http.get({url: 'http://47.106.222.216:18880/share_code/jxcfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
        await $.wait(1000)
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({url: `http://47.106.222.216:18880/api/jxcfd`, timeout: 30 * 1000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(JSON.stringify(err))
